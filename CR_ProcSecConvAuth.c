...
main(int argc,char **argv)
{
	...
	char						frm_seccd[APL_INSTRUMENTCODE_LEN]     = APL_NULL_STRING;
	...
	memset(frm_seccd,NULL,sizeof(frm_seccd));
	...
	strcat(chr_l_logname, "SECCONV_AUTH.log");
	...
	int_retval = CR_Proc_SecConvAuth(frm_seccd,chr_auth_reset_flg,argv,l_ptr_log_file,&l_intl_envdatastruct_h,&l_debug_list);
	...
	if (int_retval != APL_SUCCESS)
	{
		...
		CO_ProcMonitor(l_ptr_log_file,"FAILURE from Security Conversion Authorisation program \n",NULL,&l_intl_envdatastruct_h);
	...
	else
	{
		...
		CO_ProcMonitor(l_ptr_log_file,"SUCCESS from Security Conversion Authorisation program \n",NULL,&l_intl_envdatastruct_h);
...
int CR_Proc_SecConvAuth(char *p_from_secode,char *p_auth_reset_flg,char **argv,FILE *p_ptr_log_file,INTL_ENV_DATA_STRUCT_H *p_intl_env_data_struct_h_d,DEBUG_INFO_STRUCT_H **l_debug_info_ptr)
{
	int							int_retval = 0;
	char							chr_l_buf[BUFFER_LEN] = APL_NULL_STRING;
	char							chr_iexist,chr_iauthorised,chr_iconverted;

	CO_ProcMonitor(p_ptr_log_file,"Entered processtion CR_Proc_SecConvAuth \n",NULL,p_intl_env_data_struct_h_d);

	if (strlen(p_from_secode) == 0)
	...
	int_retval = CO_Chk_SecValid(p_from_secode,chr_iexist,chr_iauthorised,NULL,NULL,NULL,chr_iconverted,l_debug_info_ptr);

	if (int_retval != APL_SUCCESS)
		APL_GOBACK_FAIL

	int_retval = CR_Proc_DBSecConvAuth(p_from_secode,p_auth_reset_flg,argv,p_ptr_log_file,p_intl_env_data_struct_h_d,l_debug_info_ptr);
	...
	RETURN_SUCCESS :
	{
		CO_ProcMonitor(p_ptr_log_file,"Leaving processtion CR_Proc_SecConvAuth successfully \n",NULL,p_intl_env_data_struct_h_d);
		return(APL_SUCCESS);
	}

	RETURN_FAILURE  :
	{
		CO_ProcMonitor(p_ptr_log_file,"Leaving processtion CR_Proc_SecConvAuth with failure \n",NULL,p_intl_env_data_struct_h_d);
		return(APL_FAILURE);
	}
}
