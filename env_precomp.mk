...
# Entering /build/s624/buildtools/mkfile/prefix.mk

# Entering /build/s624/buildtools/mkfile/defs.mk
...
# folllowing line for AIX OS
#LDLIBS=$(EXPDLIBS) $(EXOSLIBS) $(SYSLIBS) $(EXSYSLIBS) $(MATHLIB) $(USRLIBS) -maix64 -Wl,-bbigtoc
#Enble folllowing line for Su OS or LINUX OS
LDLIBS=$(EXPDLIBS) $(EXOSLIBS) $(SYSLIBS) $(EXSYSLIBS) $(MATHLIB) $(USRLIBS) -m64
MATHLIB=-lm
LINKLDLIBS=$(LDLIBS)
COBLDLIBS=$(LDLIBS)
...
#Enable following line for Sun-OS or linux OS
SHARED_CLIENTLIBS=-L$(LINUXCLN) $(LLIBCLNTSH) $(LDFLAGSLIST)
...
# Exiting /build/s624/buildtools/mkfile/defs.mk
# Entering /build/s624/buildtools/mkfile/platform.mk
...
JAVA11HOME=/usr/local/packages/jdk1.1.8/
JAVACLASSPATH=$(JAVAHOME)/lib/classes.zip
JAVA10HOME=/usr/local/packages/java-1.0.2/
JAVA12HOME=/usr/local/packages/jdk1.2.2/
JAVA13HOME=/usr/local/packages/jdk1.3.1/
JAVA14HOME=/usr/local/packages/j2sdk1.4.1/

JRE11HOME=/usr/local/packages/jre1.1.8/
JRE12HOME=$(JAVA12HOME)jre/
JRE13HOME=$(JAVA13HOME)jre/
JRE14HOME=$(JAVA14HOME)jre/

GROUP=/etc/group

LINK=$(ADE_DEL_FILE_CMD) $(PURECMDS) $(LD) $(LDFLAGS) -dy $(COMPSOBJS)
LINKCC=$(ADE_DEL_FILE_CMD) $(PURECMDS) $(CCC) $(LDFLAGS) -xarch=v9 -norunpath -mt -nocx -dy $(COMPOBJSCC)
...
#Comment for Linux
#SHAREDLIBPATH = -Wl,-R $(ORACLE_HOME)/$(LIBDIR)
EXSYSLIBS= $(SHAREDLIBPATH) $(LLAIO) $(THREADLIBS) $(LLIBPOSIX4) $(LLIBKSTAT)

LLPPATH=$(LD_LIBRARYPATH):
LINKEXSYSLIBS32= -R $(CLUSTERDIR)/lib:$(ORACLE_HOME)/$(LIBDIR):$(ORACLUSTERDIR) \
	-Y P,$(LLPPATH)$(CLUSTERDIR)/lib:$(ORACLUSTERDIR):/usr/ccs/lib:/usr/lib \
	-Qy -lc -laio $(LLIBPOSIX4) $(LLIBKSTAT)
LINKEXSYSLIBSCC= -R $(CLUSTERDIR)/lib:$(ORACLE_HOME)/lib:$(ORACLUSTERDIR) \
	-Y P,$(LLPPATH)$(CLUSTERDIR)/lib:$(ORACLUSTERDIR):/usr/ccs/lib:/usr/lib \
...
LINKEXSYSLIBS64= -R $(CLUSTERDIR)/lib/sparcv9:$(ORACLE_HOME)/$(LIBDIR):$(ORACLUSTERDIR) \
        -Y P,$(LLPPATH)$(CLUSTERDIR)/lib/sparcv9:$(ORACLUSTERDIR):/usr/ccs/lib/sparcv9:/usr/lib/sparcv9\
        -Qy -lc -laio $(LLIBPOSIX4) $(LLIBKSTAT)
...
SHLIB_OBJS=-z allextract $(PRODLIBHOME)$(_LIBNAME).$(LIB_EXT) -z defaultextract
SHLIB_OBJS_OLD=-z allextract $(PRODLIBHOME)$(_LIBNAME).$(LIB_EXT)
...
# Exiting /build/s624/buildtools/mkfile/platform.mk
# Entering /build/s624/buildtools/mkfile/rules.mk

# Exiting /build/s624/buildtools/mkfile/rules.mk
# Exiting /build/s624/buildtools/mkfile/prefix.mk
# Entering /build/s624/ldap/exports.mk
...
# Entering /build/s624/ldap/s_exports.mk


# Exiting /build/s624/ldap/s_exports.mk
# Exiting /build/s624/ldap/exports.mk
# Entering /build/s624/encryption/exports.mk
...
# Entering /build/s624/encryption/s_exports.mk

# Exiting /build/s624/encryption/s_exports.mk
# Exiting /build/s624/encryption/exports.mk
# Entering /build/s624/network/exports.mk
...
# Entering /build/s624/network/adapters.mk

# Entering /build/s624/network/s_adapters.mk

# Exiting /build/s624/network/s_adapters.mk

# Entering /build/s624/network/libnames.mk
...
# Exiting /build/s624/network/libnames.mk

# Exiting /build/s624/network/adapters.mk
...
# Entering /build/s624/network/s_exports.mk
...
# Exiting /build/s624/network/s_exports.mk
# Exiting /build/s624/network/exports.mk
# Entering /build/s624/oracore/exports.mk
...
# Entering /build/s624/oracore/s_exports.mk

# Exiting /build/s624/oracore/s_exports.mk
# Exiting /build/s624/oracore/exports.mk
# Entering /build/s624/nlsrtl/exports.mk
...
# Entering /build/s624/nlsrtl/s_exports.mk
...
# Exiting /build/s624/nlsrtl/s_exports.mk
# Exiting /build/s624/nlsrtl/exports.mk
# Entering /build/s624/rdbms/exports.mk
...
#Enable following line for LINUX OS
LINUXCLN=/home/Automation/intellect/build/batch/

LIBCLNTSHNAME=clntsh
#Following line is used onlu for AIX OS
#LIBCLNTSH=$(LIBHOME)$(LIB_PREFIX)$(LIBCLNTSHNAME).$(SO_EXT)

#Enable following line for LINUX OS
LIBCLNTSH=$(LINUXCLN)$(LIB_PREFIX)$(LIBCLNTSHNAME).$(SO_EXT)
...
# Entering /build/s624/rdbms/s_exports.mk
...
# Exiting /build/s624/rdbms/s_exports.mk
# Exiting /build/s624/rdbms/exports.mk
# Entering /build/s624/plsql/exports.mk
...
# Entering /build/s624/plsql/s_exports.mk

# Exiting /build/s624/plsql/s_exports.mk
# Exiting /build/s624/plsql/exports.mk
# Entering /build/s624/slax/exports.mk
...
# Entering /build/s624/slax/s_exports.mk

# Exiting /build/s624/slax/s_exports.mk
# Exiting /build/s624/slax/exports.mk
# Entering /build/s624/xdk/exports.mk
...
# Entering /build/s624/xdk/s_exports.mk

# Exiting /build/s624/xdk/s_exports.mk



# Exiting /build/s624/xdk/exports.mk
# Entering /build/s624/javavm/exports.mk
LIBJAVAVM=$(LIBHOME)$(LIB_PREFIX)aurora.$(LIB_EXT)
LLIBJAVAVM=$(LDLIBFLAG)aurora

# Entering /build/s624/javavm/s_exports.mk

# Exiting /build/s624/javavm/s_exports.mk
# Exiting /build/s624/javavm/exports.mk

# Entering /build/s624/dbjava/exports.mk
...
# Exiting /build/s624/dbjava/exports.mk

# Entering /build/s624/ctx/exports.mk
...
# Entering /build/s624/ctx/s_exports.mk
# Exiting /build/s624/ctx/s_exports.mk
# Exiting /build/s624/ctx/exports.mk
# Entering /build/s624/javavm/exports.mk
LIBJAVAVM=$(LIBHOME)$(LIB_PREFIX)aurora.$(LIB_EXT)
LLIBJAVAVM=$(LDLIBFLAG)aurora
...
# Exiting /build/s624/javavm/s_exports.mk
# Exiting /build/s624/javavm/exports.mk
# Entering /build/s624/sqlplus/exports.mk
...
# Entering /build/s624/sqlplus/s_exports.mk

# For LINUX Enable Below
SHARED_LDFLAG=-G -L$(SQLPLUSHOME)lib -L$(ORACLE_HOME)/lib  -o
#For AIX and other enable below
...
# Exiting /build/s624/sqlplus/s_exports.mk
# Exiting /build/s624/sqlplus/exports.mk

# Entering /build/s624/precomp/exports.mk
...
# Entering /build/s624/precomp/s_exports.mk

# Exiting /build/s624/precomp/s_exports.mk
# Exiting /build/s624/precomp/exports.mk
...
# Entering /build/s624/precomp/s_precomp.mk

LLAIO=
#Following line is used by AIX OS
#LLIBTHREAD=-lthread
...
PRODLIBHOME32=$(PRECOMPHOME)$(LIBDIR32)/

SYS_INCLUDE='sys_include=(/usr/include)'

CPLUSPLUS=$(CCC) -mt ${LFLAGS64}
CPLUS_SYS_INCLUDE='sys_include=($(ORACLE_HOME)/precomp/public,/opt/SUNWspro6u1/WS6U1/include/CC4,/usr/include)'
...
COB2GNT_32=$(COB32) $(COBGNTFLAGS) $*.cob
DEMO_PROCOB_BUILD_SHARED_32=$(COB32) $(COBFLAGS) -o $(EXE) $(COBS) $(LDPATHFLAG)$(LIBHOME) $(COBSQLINTF) $(COBOL_PROLDLIBS)
DEMO_PROCOB_BUILD_STATIC_32=$(COB32) $(COBFLAGS) -o $(EXE) $(COBS) $(LDPATHFLAG)$(LIBHOME) $(COBSQLINTF) $(COBOL_STATICPROLDLIBS)
...
SHIPHOME_JDK_LOCATION=$(JAVA14HOME)
...
PROCOB32=procob32$(EXE_SUFFIX)
PROCOB18_32=procob18_32$(EXE_SUFFIX)
PROFOR32=profor32$(EXE_SUFFIX)
RTSORA32=rtsora32$(EXE_SUFFIX)

PL_LINK_OVERRIDES=PROCOB=$(PROCOB32) \
		  PROCOB18=$(PROCOB18_32) \
		  PROFOR=$(PROFOR) \
		  RTSORA=$(RTSORA32) \
                  LIBDIR=$(LIBDIR32) \
		  CFLAGS="$(NOKPIC_CCFLAGS32)" \
		  COMPSOBJS="$(COMPSOBJS32INS)" \
		  COMPEOBJS="$(COMPEOBJS32INS)" \
		  LINKEXSYSLIBS="$(LINKEXSYSLIBS32)" \
		  LDFLAGS="-o $(PRODLIBHOME32)$@ $(LDFLAGS32)" \
		  COBMODE=32

INSTALL_SHORTCUT_TARGETS=$(MODADA) $(OTT8) $(PROC) $(PROCOB) $(RTSORA) $(PROPLI)
INSTALL_SHORTCUT_32_64_TARGETS=$(PROCOB32) $(PROCOB18_32) $(PROFOR) $(RTSORA32)
INSTALL_RENAMED_32_64_TARGETS=$(PROFOR)

LINK_SHORTCUT_TARGETS=$(MODADA) $(OTT8) $(PROC) $(PROCOB) \
                      $(PROPLI) $(RTSORA) $(FIDL)
LINK_SHORTCUT_32_64_TARGETS=$(PROFOR) $(PROCOB_32) $(RTSORA_32)
LINK_RENAMED_32_64_TARGETS=$(PROFOR)

DEMO_PROC_BUILD_STATIC_64=$(CC) $(LFLAGS64) -o $(EXE) $(OBJS) $(LDPATHFLAG)$(LIBHOME)  -lkstat -lnnz10  $(STATICPROLDLIBS)
DEMO_PROC_CPPBUILD_STATIC_64=$(CPLUSPLUS) $(LFLAGS64) -o $(EXE) $(OBJS) $(LDPATHFLAG)$(LIBHOME) -lkstat -lnnz10 $(STATICCPPLDLIBS)
...
# Exiting /build/s624/precomp/s_precomp.mk
# Exiting precomp.mk
# Entering /build/s624/buildtools/mkfile/libclntsh.mk
...
# Exiting /build/s624/buildtools/mkfile/libclntsh.mk
