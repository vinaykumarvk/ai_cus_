...
int MT_ModPtyFundDet (MT_PTY_FUND_DET_STRUCT_H *p_mt_pty_fund_det_struct_h, INTL_ENV_DATA_STRUCT_H *p_intl_env_data_struct_h,DEBUG_INFO_STRUCT_H **l_debug_info_ptr)
{
	...
	l_key_values[0]=p_mt_pty_fund_det_struct_h -> h_pty_code;
   l_key_values[1] = APL_NULL_STRING;
	...
	/*	Mandatory Data Missing Checks	- For Character */
			
  /* if ((p_mt_pty_fund_det_struct_h->h_acc_typ !='C')&&(p_mt_pty_fund_det_struct_h->h_acc_typ !='E')&&(p_mt_pty_fund_det_struct_h->h_acc_typ !='F')&&(p_mt_pty_fund_det_struct_h->h_acc_typ !='N')&&(p_mt_pty_fund_det_struct_h->h_acc_typ !='R'))
   {
      APL_DATA_MISSING("Account Type cannot be NULL",APL_NULL_STRING,APL_NULL_STRING);
 ...
 if(!strcmp(p_intl_env_data_struct_h->h_mode,APL_FUNC_INPUT))
	{
		APL_IF_DEBUG
 	{
  		CO_ProcMonitor(APL_OUT_FILE, "Entered input mode of MT_ModPtyFundDet... \n",NULL,p_intl_env_data_struct_h);
 	}

		MT_ModDBPtyFundDet(p_mt_pty_fund_det_struct_h ,p_intl_env_data_struct_h ,l_debug_info_ptr);
 ...
 else if(!strcmp(p_intl_env_data_struct_h->h_mode,APL_FUNC_MODIFY))
	{
		APL_IF_DEBUG
 	{
  		CO_ProcMonitor(APL_OUT_FILE, "Entered modify mode of MT_ModPtyFundDet... \n",NULL,p_intl_env_data_struct_h);
 	}
 ...
 else if(!strcmp(p_intl_env_data_struct_h->h_mode,APL_FUNC_DELETE))
	{
		APL_IF_DEBUG
 	{
  		CO_ProcMonitor(APL_OUT_FILE, "Entered delete mode of MT_ModPtyFundDet... \n",NULL,p_intl_env_data_struct_h);
 	}
...
AIX -Warning Removal - removed coment with in comment
		if (int_retval != APL_SUCCESS)
    {
  ...
  else if(!strcmp(p_intl_env_data_struct_h->h_mode,APL_FUNC_AUTHORISE))
	 {
		APL_IF_DEBUG
 	{
  		CO_ProcMonitor(APL_OUT_FILE, "Entered authorize mode of MT_ModPtyFundDet... \n",NULL,p_intl_env_data_struct_h);
 	}
    ...
    if (int_retval_mkrchkr !=APL_SUCCESS)
  ...
  else if(!strcmp(p_intl_env_data_struct_h->h_mode,APL_FUNC_RESET))
	 {
		APL_IF_DEBUG
 	{
  		CO_ProcMonitor(APL_OUT_FILE, "Entered reset mode of MT_ModPtyFundDet... \n",NULL,p_intl_env_data_struct_h);
 	}
