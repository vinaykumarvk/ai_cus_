...
int MT_UpdCA_NIS_ALLOTMENT (CA_NIS_ALLOTMENT_STRUCT_H *p_ca_nis_allotment_struct_h, INTL_ENV_DATA_STRUCT_H *p_intl_env_data_struct_h,DEBUG_INFO_STRUCT_H **l_debug_info_ptr)
{
...
printf("Before allmand check clncode |%s|\n",p_ca_nis_allotment_struct_h -> h_cln_code);
 ...
 if (strcmp(p_intl_env_data_struct_h->h_mode,APL_FUNC_INPUT))
	{
		if (!strlen(p_ca_nis_allotment_struct_h->h_access_stamp)) 
		{
 ...
 if(!strcmp(p_intl_env_data_struct_h->h_mode,APL_FUNC_INPUT))
	{
		APL_IF_DEBUG
 	{
  		CO_ProcMonitor(APL_OUT_FILE, "Entered input mode of MT_UpdCA_NIS_ALLOTMENT... \n",NULL,p_intl_env_data_struct_h);
 	}

		MT_ModDBCA_NIS_ALLOTMENT(p_ca_nis_allotment_struct_h ,p_intl_env_data_struct_h ,l_debug_info_ptr);
 ...
 else if(!strcmp(p_intl_env_data_struct_h->h_mode,APL_FUNC_MODIFY))
	{
		APL_IF_DEBUG
 	{
  		CO_ProcMonitor(APL_OUT_FILE, "Entered modify mode of MT_UpdCA_NIS_ALLOTMENT... \n",NULL,p_intl_env_data_struct_h);
 	}
 ...
 else if(!strcmp(p_intl_env_data_struct_h->h_mode,APL_FUNC_DELETE))
	{
		APL_IF_DEBUG
 	{
  		CO_ProcMonitor(APL_OUT_FILE, "Entered delete mode of MT_UpdCA_NIS_ALLOTMENT... \n",NULL,p_intl_env_data_struct_h);
 	}
	   MT_ModDBCA_NIS_ALLOTMENT(p_ca_nis_allotment_struct_h ,p_intl_env_data_struct_h ,l_debug_info_ptr);	
  ...
  else if(!strcmp(p_intl_env_data_struct_h->h_mode,APL_FUNC_AUTHORISE))
	 {
		APL_IF_DEBUG
 	{
  		CO_ProcMonitor(APL_OUT_FILE, "Entered authorize mode of MT_UpdCA_NIS_ALLOTMENT... \n",NULL,p_intl_env_data_struct_h);
 	}
    ...
    if (int_retval_mkrchkr !=APL_SUCCESS)
  ...
  else if(!strcmp(p_intl_env_data_struct_h->h_mode,APL_FUNC_RESET))
	 {
		APL_IF_DEBUG
 	{
  		CO_ProcMonitor(APL_OUT_FILE, "Entered reset mode of MT_UpdCA_NIS_ALLOTMENT... \n",NULL,p_intl_env_data_struct_h);
 	}
