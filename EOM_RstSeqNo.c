...
main(int argc,char **argv)
{
	...
	strcat(chr_l_logname, "RST_SEQ_NO_M.log");
	...
	if (CO_DB_Connection_G(chr_uname,chr_passwd,l_ptr_log_file,&l_debug_list) != APL_SUCCESS)
		exit(-1);	
	...
	int_retval = EOM_Proc_SqNoRst(argv,l_ptr_log_file,&l_intl_envdatastruct_h,&l_debug_list);
...
int EOM_Proc_SqNoRst(char **argv,FILE *p_ptr_log_file,INTL_ENV_DATA_STRUCT_H *p_intl_env_data_struct_h_d,DEBUG_INFO_STRUCT_H **l_debug_info_ptr)
{
	int							int_retval = 0;
	char							chr_l_buf[BUFFER_LEN] = APL_NULL_STRING;

	CO_ProcMonitor(p_ptr_log_file,"Entered processtion EOM_Proc_SqNoRst \n",NULL,p_intl_env_data_struct_h_d);
	...
	int_retval = EOM_Proc_DBSqNoRst(p_ptr_log_file,p_intl_env_data_struct_h_d,l_debug_info_ptr);
	if (int_retval != APL_SUCCESS)
		APL_GOBACK_FAIL
	...
	RETURN_SUCCESS :
	{
		CO_ProcMonitor(p_ptr_log_file,"Leaving processtion EOM_Proc_SqNoRst \n",NULL,p_intl_env_data_struct_h_d);
