...
int MT_ModDlFlRate (MT_DLFLRATE_STRUCT_H *p_mt_dlflrate_struct_h, INTL_ENV_DATA_STRUCT_H *p_intl_env_data_struct_h,DEBUG_INFO_STRUCT_H **l_debug_info_ptr)
{
	...
	if (p_mt_dlflrate_struct_h->h_float_rate1 == 0) 
	{
		APL_DATA_MISSING("Rate Cannot be 0",APL_NULL_STRING,APL_NULL_STRING);
 ...
 if (strcmp(p_intl_env_data_struct_h->h_mode,APL_FUNC_INPUT))
	{
		if (!strlen(p_mt_dlflrate_struct_h->h_access_stamp)) 
		{
 ...
 if(!strcmp(p_intl_env_data_struct_h->h_mode,APL_FUNC_INPUT))
	{
		APL_IF_DEBUG
 	{
		...
		MT_ModDBDlFlRate(p_mt_dlflrate_struct_h ,p_intl_env_data_struct_h ,l_debug_info_ptr);
