 ...
 *
 * File Name           :      DL_ModSingle_Prematch_Detail.c
 *
...
int DL_ModSingle_Prematch(DL_SINGLE_PREMATCH_STRUCT_H *p_dl_single_prematch_struct_h, INTL_ENV_DATA_STRUCT_H *p_intl_env_data_struct_h,DEBUG_INFO_STRUCT_H **l_debug_info_ptr)
{
	printf("JC::: In function DL_ModSingle_Prematch in DL_ModSingle_Prematch_Detail.c\n");
	...
	printf("JC::: Mandatory Data Checks function called CR_Chk_AllMandData\n");
	...
	/*	Mandatory Data Missing Checks	- For Numbers */
   	if (strlen(p_dl_single_prematch_struct_h->h_dl_client) == 0)
   	{
    ...
    {
          APL_DATA_MISSING("Deal Reference Number cannot be null",APL_NULL_STRING,APL_NULL_STRING);
          int_error_flag = APL_FAILURE;
    }
  /* IB_4677 Start */
  if (p_dl_single_prematch_struct_h->h_newquantity == 0)
    {
          APL_DATA_MISSING("Qty can not be 0",APL_NULL_STRING,APL_NULL_STRING);
          int_error_flag = APL_FAILURE;
    }
  /* IB_4677 End */
	...
	/* Commented by JC - Not required IB_PH3_53 [Start]
	l_key_codes[0]="DL_CLIENT";
	l_key_codes[1]="IDENTIY_NO";
	l_key_codes[2]=APL_NULL_STRING;
	...
	if (int_retval !=APL_SUCCESS)
	 ...
	 Commented by JC - Not required IB_PH3_53 [End] */
	...
	if(!strcmp(p_intl_env_data_struct_h->h_mode,APL_FUNC_INPUT))
	{
		APL_IF_DEBUG
 		{
  			CO_ProcMonitor(APL_OUT_FILE, "Entered input mode of DL_ModSingle_Prematch... \n",NULL,p_intl_env_data_struct_h);
 		}

		printf("JC::: In the INPUT mode of DL_ModSingle_Prematch & calling DL_UpdSingle_Prematch\n");
		DL_UpdSingle_Prematch(p_dl_single_prematch_struct_h ,p_intl_env_data_struct_h ,l_debug_info_ptr);
	...
	else if(!strcmp(p_intl_env_data_struct_h->h_mode,APL_FUNC_MODIFY))
	{
		APL_IF_DEBUG
 		{
  			CO_ProcMonitor(APL_OUT_FILE, "Entered modify mode of DL_ModSingle_Prematch... \n",NULL,p_intl_env_data_struct_h);
 		}
	...
	else if(!strcmp(p_intl_env_data_struct_h->h_mode,APL_FUNC_DELETE))
	{
		APL_IF_DEBUG
 		{
  			CO_ProcMonitor(APL_OUT_FILE, "Entered delete mode of DL_ModSingle_Prematch... \n",NULL,p_intl_env_data_struct_h);
 		}
	...
	else if(!strcmp(p_intl_env_data_struct_h->h_mode,APL_FUNC_AUTHORISE))
	{
		APL_IF_DEBUG
 		{
  			CO_ProcMonitor(APL_OUT_FILE, "Entered authorize mode of DL_ModSingle_Prematch... \n",NULL,p_intl_env_data_struct_h);
 		}
	...
	else if(!strcmp(p_intl_env_data_struct_h->h_mode,APL_FUNC_RESET))
	 {
		APL_IF_DEBUG
 		{
  			CO_ProcMonitor(APL_OUT_FILE, "Entered reset mode of DL_ModSingle_Prematch... \n",NULL,p_intl_env_data_struct_h);
 		}
