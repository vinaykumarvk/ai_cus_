...
int IV_ModManual_Msg (MANUAL_MESSAGE_STRUCT_H *p_manual_message_struct_h,MS_MANMSG_STRUCT_H *p_ms_manmsg_struct_h, char *event_id, INTL_ENV_DATA_STRUCT_H *p_intl_env_data_struct_h,DEBUG_INFO_STRUCT_H **l_debug_info_ptr)
{
	printf("######### Entered IV_ModManual_Msg\n");
	...
	/* Not required..... So commented -- JC - RFS_MS_001
	   if (p_manual_message_struct_h->h_fld_no_1 == 0)
	   {
	...
	/* Access stamp check not reqd here.. So commented -- JC - RFS_MS_001 
	if (strcmp(p_intl_env_data_struct_h->h_mode,APL_FUNC_INPUT))
	{
		if (!strlen(p_manual_message_struct_h->h_access_stamp)) 
		{
	...
	if(!strcmp(p_intl_env_data_struct_h->h_mode,APL_FUNC_INPUT))
	{
		APL_IF_DEBUG
		{
			CO_ProcMonitor(APL_OUT_FILE, "Entered input mode of IV_ModManual_Msg... \n",NULL,p_intl_env_data_struct_h);
		}

		IV_UpdManual_Msg(p_manual_message_struct_h, p_ms_manmsg_struct_h, event_id, p_intl_env_data_struct_h ,l_debug_info_ptr);
	...
	else if(!strcmp(p_intl_env_data_struct_h->h_mode,APL_FUNC_MODIFY))
	{
		APL_IF_DEBUG
		{
			CO_ProcMonitor(APL_OUT_FILE, "Entered modify mode of IV_ModManual_Msg... \n",NULL,p_intl_env_data_struct_h);
		}
	...
	else if(!strcmp(p_intl_env_data_struct_h->h_mode,APL_FUNC_DELETE))
	{
		APL_IF_DEBUG
		{
			CO_ProcMonitor(APL_OUT_FILE, "Entered delete mode of IV_ModManual_Msg... \n",NULL,p_intl_env_data_struct_h);
		}
	...
	else if(!strcmp(p_intl_env_data_struct_h->h_mode,APL_FUNC_AUTHORISE))
	{
		APL_IF_DEBUG
		{
			CO_ProcMonitor(APL_OUT_FILE, "Entered authorize mode of IV_ModManual_Msg... \n",NULL,p_intl_env_data_struct_h);
		}
	 ...
	 else if(!strcmp(p_intl_env_data_struct_h->h_mode,APL_FUNC_RESET))
	 {
		APL_IF_DEBUG
		{
			CO_ProcMonitor(APL_OUT_FILE, "Entered reset mode of IV_ModManual_Msg... \n",NULL,p_intl_env_data_struct_h);
		}
