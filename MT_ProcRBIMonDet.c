...
int MT_ModRBIMonDet (MT_RBIMONDET_STRUCT_H *p_mt_rbimondet_struct_h, INTL_ENV_DATA_STRUCT_H *p_intl_env_data_struct_h,DEBUG_INFO_STRUCT_H **l_debug_info_ptr)
{
	...
	l_key_codes[1]="RBI_INSTR_CODE";
	l_key_codes[2]=APL_NULL_STRING;
	
	l_key_values[0]=p_mt_rbimondet_struct_h->h_instr_code;
	l_key_values[1]=p_mt_rbimondet_struct_h->h_rbi_instr_code;
...
/*	if (p_mt_rbimondet_struct_h->h_tpcapital == 0) 
	{
		APL_DATA_MISSING("Total Paidup Capital Cannot be 0",APL_NULL_STRING,APL_NULL_STRING);
		int_error_flag = APL_FAILURE;
	}
	if (p_mt_rbimondet_struct_h->h_sng_fii_limit == 0) 
	{
		APL_DATA_MISSING("Single FII Limit Cannot be 0",APL_NULL_STRING,APL_NULL_STRING);
		int_error_flag = APL_FAILURE;
	}
	if (p_mt_rbimondet_struct_h->h_ovr_fii_limit == 0) 
	{
		APL_DATA_MISSING("Overall FII Cannot be 0",APL_NULL_STRING,APL_NULL_STRING);
		int_error_flag = APL_FAILURE;
	}
	if (p_mt_rbimondet_struct_h->h_sng_nriocb_limit == 0) 
	{
		APL_DATA_MISSING("Single NRI OCB Limit Cannot be 0",APL_NULL_STRING,APL_NULL_STRING);
		int_error_flag = APL_FAILURE;
	}
	if (p_mt_rbimondet_struct_h->h_ovr_nriocb_limit == 0) 
	{
		APL_DATA_MISSING("Overall NRI OCB Limit Cannot be 0",APL_NULL_STRING,APL_NULL_STRING);
		int_error_flag = APL_FAILURE;
	}
	if (p_mt_rbimondet_struct_h->h_caution_fii == 0) 
	{
		APL_DATA_MISSING("Caution percentage for FII Cannot be 0",APL_NULL_STRING,APL_NULL_STRING);
		int_error_flag = APL_FAILURE;
	}
	if (p_mt_rbimondet_struct_h->h_caution_nriocb == 0) 
	{
		APL_DATA_MISSING("Caution percentage for NRI OCB Cannot be 0",APL_NULL_STRING,APL_NULL_STRING);
		int_error_flag = APL_FAILURE;
	}*/
 ...
 if (strcmp(p_intl_env_data_struct_h->h_mode,APL_FUNC_INPUT))
	{
		if (!strlen(p_mt_rbimondet_struct_h->h_access_stamp)) 
		{
 ...
 if(!strcmp(p_intl_env_data_struct_h->h_mode,APL_FUNC_INPUT))
	{
		APL_IF_DEBUG
 	{
		...
		MT_ModDBRBIMonDet(p_mt_rbimondet_struct_h ,p_intl_env_data_struct_h ,l_debug_info_ptr);
