 ...
 *
 * File Name           :      MT_Populate_CA_PUTCALL_APP.c
 *
...
int CA_PUTCALL_APP(CA_PUTCALL_APP_STRUCT_H *p_ca_putcall_app_struct_h, INTL_ENV_DATA_STRUCT_H *p_intl_env_data_struct_h,DEBUG_INFO_STRUCT_H **l_debug_info_ptr)
{
  ...
  /* if (p_ca_putcall_app_struct_h->h_fld_no_1 == 0)
   {
 ...
 if (strcmp(p_intl_env_data_struct_h->h_mode,APL_FUNC_INPUT))
	{
		if (!strlen(p_ca_putcall_app_struct_h->h_access_stamp)) 
		{
 ...
 if(!strcmp(p_intl_env_data_struct_h->h_mode,APL_FUNC_INPUT))
	{
		APL_IF_DEBUG
 	{
  		CO_ProcMonitor(APL_OUT_FILE, "Entered input mode of CA_PUTCALL_APP_STRUCT_H... \n",NULL,p_intl_env_data_struct_h);
 	}

		CA_ModDbCA_PUTCALL_APP(p_ca_putcall_app_struct_h ,p_intl_env_data_struct_h ,l_debug_info_ptr);
 ...
 else if(!strcmp(p_intl_env_data_struct_h->h_mode,APL_FUNC_MODIFY))
	{
		APL_IF_DEBUG
 	{
  		CO_ProcMonitor(APL_OUT_FILE, "Entered modify mode of CA_PUTCALL_APP_STRUCT_H... \n",NULL,p_intl_env_data_struct_h);
 	}
 ...
 else if(!strcmp(p_intl_env_data_struct_h->h_mode,APL_FUNC_DELETE))
	{
		APL_IF_DEBUG
 	{
  		CO_ProcMonitor(APL_OUT_FILE, "Entered delete mode of CA_PUTCALL_APP_STRUCT_H... \n",NULL,p_intl_env_data_struct_h);
 	}
		...
		if (int_retval != APL_SUCCESS)
    {
  ...
  else if(!strcmp(p_intl_env_data_struct_h->h_mode,APL_FUNC_AUTHORISE))
	 {
		APL_IF_DEBUG
 	{
  		CO_ProcMonitor(APL_OUT_FILE, "Entered authorize mode of CA_PUTCALL_APP_STRUCT_H... \n",NULL,p_intl_env_data_struct_h);
 	}
    ...
    if (int_retval_mkrchkr !=APL_SUCCESS)
  ...
  else if(!strcmp(p_intl_env_data_struct_h->h_mode,APL_FUNC_RESET))
	 {
		APL_IF_DEBUG
 	{
  		CO_ProcMonitor(APL_OUT_FILE, "Entered reset mode of CA_PUTCALL_APP_STRUCT_H... \n",NULL,p_intl_env_data_struct_h);
 	}
